...
from asper.utils import setup_logger, export_solution
...
async def main() -> None:
    ...
    try:
        ...
        logger.info(f"Configuration: model={config.model_name}, max_iterations={config.max_iterations}")
        ...
        if result:
            ...
            # Log final status
            if result.success:
                logger.info(f"Problem solved successfully in {result.iterations} iterations")
            else:
