...
from asper.utils import export_solution, setup_logger, reset_logger
...
def find_problem_files(
    ...
    if years:
        ...
        candidates = [
            ...
            if p.is_dir() and p.name.startswith("lpcp-")
        ]
...
async def main() -> None:
    ...
    print(f"Configuration: model={config.model_name}, max_iterations={config.max_iterations}")
    print("-" * 60)
