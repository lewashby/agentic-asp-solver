...
class ASPRunner:
    ...
    async def _create_app_with_tools(self, tools):
        ...
        # Load prompts
        solver_prompt = PromptManager.get_solver_prompt(
            self.config.solver_prompt_file
        )
        if self.config.solver_prompt_file:
            self.logger.info(f"Loaded Solver system prompt: {self.config.solver_prompt_file}")
        else:
        ...
        if self.config.validator_prompt_file:
            self.logger.info(f"Loaded Validator system prompt: {self.config.validator_prompt_file}")
        else:
